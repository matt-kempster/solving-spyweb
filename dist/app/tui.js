...
import { applyObservedEvent } from "../solver/replay.js";
...
let state = { belief: fullBelief(universe), trace: [] };
...
function showStatus() {
    ...
    for (const score of rankQuestions(universe, state.belief).slice(0, 5)) {
        ...
        stdout.write(`  ${spy?.name ?? question.spy} ${question.sense}: worst ${score.worstPairs} pairs / ${score.worstBoards.toLocaleString()} boards\n`);
    }
}
        ...
        while (true) {
            ...
            if (command === "t") {
                for (const step of state.trace) {
                    stdout.write(`${step.sequence}. ${step.event.type}: ${step.boardsBefore} -> ${step.boardsAfter} boards, ${step.pairsBefore} -> ${step.pairsAfter} pairs\n`);
                }
                continue;
            }
            if (command !== "a")
            ...
            if (spy === undefined)
                throw new Error(`Unknown spy: ${spyName}`);
            const sense = (await rl.question("Sense (look/hear/point): ")).trim().toLowerCase();
            if (!["look", "hear", "point"].includes(sense))
