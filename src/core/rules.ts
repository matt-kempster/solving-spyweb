...
export function validateRules(rules: Rules): void {
  ...
  rules.spies.forEach((spy, index) => {
    if (spy.id !== index) throw new Error(`Spy ids must be dense; found ${spy.id} at ${index}`);
  });
  rules.cities.forEach((city, index) => {
    if (city.id !== index) throw new Error(`City ids must be dense; found ${city.id} at ${index}`);
  });
}

export function validateBoard(rules: Rules, board: Board): void {
  validateRules(rules);
  ...
  const occupants = board.occupantByCity.filter((spy): spy is SpyId => spy !== null);
  if (occupants.length !== rules.spies.length - 1) throw new Error("Board must contain every non-ringleader spy");
  if (occupants.includes(board.ringleader)) throw new Error("Ringleader must not be on the board");
  if (new Set(occupants).size !== occupants.length) throw new Error("A spy may only occupy one city");
}

function answerDirection(rules: Rules, board: Board, spy: SpyId, direction: Direction): Answer {
  if (spy === board.ringleader) return { kind: "nothing" };

  const cityIndex = board.occupantByCity.findIndex((occupant) => occupant === spy);
  if (cityIndex < 0) throw new Error(`Visible spy ${spy} has no city`);
  const start = rules.cities[cityIndex] as City | undefined;
  if (start === undefined) throw new Error(`Unknown city ${cityIndex}`);
  ...
  if (targetCity === undefined) return { kind: "nothing" };
...
export function answerQuestion(rules: Rules, board: Board, question: Question): QuestionAnswers {
  const spy = rules.spies[question.spy];
  if (spy === undefined) throw new Error(`Unknown spy ${question.spy}`);
